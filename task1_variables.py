...
# Part a: total number of games
total_games = len(video_game_sales)
print(total_games)

# Part b: average global sales across all 20 games
total_global_sales = 0
for game in video_game_sales:
    total_global_sales += game[GLOBAL_SALES]

avg_global_sales = total_global_sales / total_games
print(f"Average global sales across all games: {avg_global_sales} million units")

# Part c: percentage of total global sales from the top game (index 0)
top_game_global_sales = video_game_sales[0][GLOBAL_SALES]
top_game_share = (top_game_global_sales / total_global_sales) * 100
print(f"Top game's share of total global sales: {top_game_share}%")
